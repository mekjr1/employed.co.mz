...
_MSISDN_RE = re.compile(r"^8[67]\d{7}$")
...
def _simulator_outcome(msisdn: str) -> tuple[str, int, str | None]:
    mapping = {
        "861111111": ("completed", 1000, None),
        "863333333": ("failed", 5000, "insufficient_funds"),
        "864444444": ("failed", 30000, "user_timeout"),
        "868888888": ("failed", 5000, "wrong_pin"),
    }
...
class EmolaProvider(PaymentProvider):
    ...
    name = "e-Mola"
    markets = ["mz"]
    simulator = bool(_setting("EMOLA_SIMULATOR", True))
    ui = {"collect": "msisdn"}
    ...
    async def initiate(
        ...
        **kwargs: Any,
        ...
        if not self.simulator:
            raise RuntimeError("emola-not-implemented")

        msisdn = _normalize_msisdn(payer_msisdn)
        if msisdn is None or not _MSISDN_RE.fullmatch(msisdn):
            raise ValueError("emola-invalid-msisdn")

        provider_ref = f"sim-emola-{secrets.token_hex(6)}"
        ...
        logger.info(
            "emola.simulated intent_id=%s job_id=%s provider_ref=%s outcome=%s delay_ms=%s",
            intent_id,
            job_id,
            provider_ref,
            outcome_status,
        ...
        return InitiateResult(kind="await", provider_ref=provider_ref, prompt="emola.prompt.check_phone")
...
try:
    registry.register(EmolaProvider())
except ValueError:
    logger.debug("emola provider already registered")
